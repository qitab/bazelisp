...
def _build_flags(ctx, add_features, verbose_level, instrument_coverage):
    ...
    # consistent between this target, its image attr, and _LIBSBCL.)
...
def _lisp_core_impl(ctx):
    ...
    # TODO(sfreilich): This bit of awkward naming is to work around a weird
    # quirk in elfinate's interface where if it doesn't work if it has an
    # input of foo.core and an output of foo.core.o (it also creates
    # foo.core, clobbering the input, and then it doesn't work).
    ...
    linking_outputs = cc_common.link(
        ...
        linking_contexts = [
            ...
            # TODO(sfreilich): Might be more ideal if this propagated from and
            # was configured with the build image.
            ctx.attr._libsbcl[CcInfo].linking_context,
...
_LISP_CORE_ATTRS.update({
    ...
    "malloc": attr.label(default = _DEFAULT_MALLOC, providers = [CcInfo]),
    ...
    "_libsbcl": attr.label(default = Label(_LIBSBCL), providers = [CcInfo]),
})
